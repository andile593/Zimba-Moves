...
async function logEvent({
  ...
  try {
    ...
    if (error.code === "P2002") {
      console.log("Duplicate event log attempt prevented:", {
        gateway,
        gatewayRef,
      });
      return null;
...
exports.initiateRefund = async (req, res, next) => {
  try {
    ...
    if (payment.refundReference) {
      return res
        .status(200)
        .json({ message: "Refund already initiated", payment });
    }
    ...
    if (!payment.gatewayReference) {
      return res
        .status(400)
        .json({ error: "Missing gateway reference for refund" });
    }
 ...
 */
exports.paystackTransferWebhook = async (req, res, next) => {
  try {
    const secret = process.env.PAYSTACK_SECRET;
...
exports.checkRefundStatus = async (req, res, next) => {
  try {
    ...
    if (!refund)
      return res
        .status(404)
        .json({ error: "No refund found for this payment" });
